...
test('profileDistance is 0 for identical profiles and positive for a differing one', () => {
  const a: DifficultyProfile = { moveCount: 20, crossingMoveCount: 1, eatCount: 2, survivingGroupCount: 2, groupsUsed: 2, expandedStates: 500 }
...
test('selectDiverseTopN prefers a lower-scoring but structurally distinct candidate over a near-duplicate', () => {
  const highA = makeCandidate({ moveCount: 30, crossingMoveCount: 3, eatCount: 3, survivingGroupCount: 2, groupsUsed: 2, expandedStates: 1000 }, 50)
  const highB = makeCandidate({ moveCount: 31, crossingMoveCount: 3, eatCount: 3, survivingGroupCount: 2, groupsUsed: 2, expandedStates: 1010 }, 49)
  const distinct = makeCandidate({ moveCount: 20, crossingMoveCount: 0, eatCount: 2, survivingGroupCount: 4, groupsUsed: 4, expandedStates: 200 }, 40)
...
test('selectDiverseTopN always fills up to min(n, candidates.length) even when every candidate is similar', () => {
  const candidates = [
    makeCandidate({ moveCount: 20, crossingMoveCount: 1, eatCount: 2, survivingGroupCount: 2, groupsUsed: 2, expandedStates: 100 }, 30),
    makeCandidate({ moveCount: 21, crossingMoveCount: 1, eatCount: 2, survivingGroupCount: 2, groupsUsed: 2, expandedStates: 100 }, 29),
    makeCandidate({ moveCount: 22, crossingMoveCount: 1, eatCount: 2, survivingGroupCount: 2, groupsUsed: 2, expandedStates: 100 }, 28),
  ]
